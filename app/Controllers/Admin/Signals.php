...
// Update : https://expressiq.co/update?app=Signals
//==================================================
class Signals extends AdminController
{
	private $page = 1;
	private $per_page = 20;
	private $model;
	private $link = "/admin/signals";
	private $breadcrumb = "";
	...
	public function index(){
		...
		$this->model->where("status","pending");
		$this->model->OrWhere("status","active");
		$this->data["data"] = $this->model->getItems();

		$this->data["action"] = "list";
		$this->breadcrumb->add('Dashboard', '/admin');
		$this->breadcrumb->add(lang('signals.modules'), $this->link);
		$this->breadcrumb->add(lang('signals.title'), '/index');
	...
	public function create($id=false){
		$this->breadcrumb->add('Dashboard', '/admin');
		$this->breadcrumb->add(lang('signals.modules'), $this->link);
		$this->breadcrumb->add(lang('signals.create'), '/index');
		$this->data['breadcrumbs'] = $this->breadcrumb->render();
