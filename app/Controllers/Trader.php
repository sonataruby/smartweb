...
class Trader extends BaseController
{
	...
	public function signals($returndata=false, $token=""){
		$db = \Config\Database::connect();
        $query = $db->query("SELECT * FROM signals where opendate >= NOW() - INTERVAL 1 DAY AND active='active' AND status != 'complete' AND status != 'cancel' ORDER BY opendate DESC LIMIT 100")->getResult();
        $arv = [];
